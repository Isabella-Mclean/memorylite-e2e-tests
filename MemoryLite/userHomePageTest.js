...
describe('UserHomePage Assertions', function() {
  ...
  it('test clicking Add redirects users to create a new entry', function(browser) {
    browser.useXpath();
  ...
  it('test clicking a presaved entry displays view or delete options', function(browser) {
  ...
  it('test clicking the view/edit button redirects to the edit entry (add) page', function(browser) {
    //click the first entry
    browser.click('tr[data-p-selectable-row="true"]:first-of-type');
  ...
  it('test user can save edits to a presaved entry', async function(browser) {
    ...
    //click the view/edit button
    browser.click(viewEditButton);
    let text = await browser.element.findByPlaceholderText('Enter your text here...').getValue();
    //add text to the main text field
    ...
    //assert the number of entries has not changed
    await browser.waitForElementVisible(
      'tr[data-p-selectable-row="true"]',
      5000
    );
    let newRows = await browser.element.findAll('tr[data-p-selectable-row="true"]').count();
    assert.equal(totalRows, newRows);

    //reopen the entry and assert the changes were saved
    browser.click('tr[data-p-selectable-row="true"]:first-of-type');
    browser.useXpath();
    browser.click(viewEditButton);
    let updatedText = text + longText;
    let savedText = await browser.element.findByPlaceholderText('Enter your text here...').getValue();
    assert.equal(updatedText, savedText);
    browser.useCss();
  });
